...
const expiredCheck = (alpha, _dir) => {
    setInterval(() => {
        ...
        Object.keys(_dir).forEach((i) => {
            const timeRemaining = _dir[i].expired - Date.now();

            // Jika waktu tersisa <= 24 jam dan belum diberi notifikasi, kirim notifikasi 24 jam sebelumnya
            if (timeRemaining <= 24 * 60 * 60 * 1000 && !_dir[i].notified) {
                console.log(`Reminder 24 jam sebelum kedaluwarsa untuk: ${_dir[i].id}`);
                alpha.sendMessage(_dir[i].id, {
                    text: `⚠️ Peringatan! Masa sewa bot di grup ini akan kedaluwarsa dalam waktu kurang dari 24 jam. Hubungi owner untuk perpanjangan bot.`
                });
                _dir[i].notified = true; // Tandai bahwa notifikasi telah dikirim
                fs.writeFileSync('./database/sewa.json', JSON.stringify(_dir, null, 2));
            }
        ...
        // Jika ada posisi yang kedaluwarsa
        if (position !== null) {
            console.log(`Sewa expired: ${_dir[position].id}`);
            alpha.sendMessage(_dir[position].id, {
                text: `Masa sewa di grup ini telah habis, bot otomatis keluar!\n\nHubungi Owner Kami Untuk Perpanjangan Bot.`
            })
            .then(async () => {
