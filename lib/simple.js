...
/**
 * Serialize Message
 * @param {WAalphaection} alpha 
 * @param {Object} m 
 * @param {store} store 
 */
exports.smsg = (alpha, m, store) => {
    if (!m) return m
    ...
    if (m.key) {
        ...
        m.sender = alpha.decodeJid(m.fromMe && alpha.user.id || m.participant || m.key.participant || m.chat || '')
        if (m.isGroup) m.participant = alpha.decodeJid(m.key.participant) || ''
    }
	if (m.message) {
...
//		m.body = m.message.conversation || m.msg.caption || m.msg.text || (m.mtype == 'listResponseMessage') && m.msg.singleSelectReply.selectedRowId || (m.mtype == 'buttonsResponseMessage') && m.msg.selectedButtonId || (m.mtype == 'viewOnceMessage') && m.msg.caption || m.tex // (m.mtype === 'conversation' && m.message.conversation) ? m.message.conversation : (m.mtype == 'imageMessage') && m.message.imageMessage.caption ? m.message.imageMessage.caption : (type == 'videoMessage') && m.message.videoMessage.caption ? m.message.videoMessage.caption : (m.mtype == 'extendedTextMessage') && m.message.extendedTextMessage.text ? m.message.extendedTextMessage.text : (m.mtype == 'listResponseMessage') && m.message.listResponseMessage.singleSelectReply.selectedRowId ? m.message.listResponseMessage.singleSelectReply.selectedRowId : (m.mtype == 'buttonsResponseMessage') && m.message.buttonsResponseMessage.selectedButtonId ? m.message.buttonsResponseMessage.selectedButtonId : (m.mtype == 'templateButtonReplyMessage') && m.message.templateButtonReplyMessage.selectedId ? m.message.templateButtonReplyMessage.selectedId : ''
		...
		if (m.quoted) {
			...
			m.quoted.sender = alpha.decodeJid(m.msg.contextInfo.participant)
			m.quoted.fromMe = m.quoted.sender === (alpha.user && alpha.user.jid)
			m.quoted.text = m.quoted.text || m.quoted.caption || m.quoted.conversation || m.quoted.contentText || m.quoted.selectedDisplayText || m.quoted.title || ''
			m.quoted.mentionedJid = m.quoted.contextInfo ? m.quoted.contextInfo.mentionedJid : []
			m.getQuotedObj = m.getQuotedMessage = async () => {
				if (!m.quoted.id) return false
				let q = await store.loadMessage(m.chat, m.quoted.id, alpha)
				return exports.smsg(alpha, q, store)
			}
			 ...
			 * @returns 
			 */
			m.quoted.delete = () => alpha.sendMessage(m.quoted.chat, {
				delete: vM.key
			})
			 ...
			 * @returns 
			 */
			m.quoted.copyNForward = (jid, forceForward = false, options = {}) => alpha.copyNForward(jid, vM, forceForward, options)

			/**
			 *
			 * @returns
			 */
			m.quoted.download = () => alpha.downloadMediaMessage(m.quoted)
		}
	}
    if (m.msg.url) m.download = () => alpha.downloadMediaMessage(m.msg)
	...
	*/
    m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text) ? alpha.sendMedia(chatId, text, 'file', '', m, { ...options }) : alpha.sendText(chatId, text, m, { ...options })
    /**
	* Copy this message
	*/
	m.copy = () => exports.smsg(alpha, M.fromObject(M.toObject(m)))
	 ...
	 * @returns 
	 */
	m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => alpha.copyNForward(jid, m, forceForward, options)
